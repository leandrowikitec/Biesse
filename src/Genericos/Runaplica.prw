...
TButton():New(17,250,"&Executar",oDlgRun,{|| If(Empty(cGet).OR.!FindFunction(cGet),bGet:={|| .T. },bGet := &("{||xRet := "+cGet+" }")), MsgRun("Executando...","Aguarde.",{||TrataExec(bGet,lProtErro)}), cResult := VarInfo('xRet',xRet,,.F.), oResult:Refresh() },45,15,,,,.T.,,,,,,)
...
TButton():New(08,230,"&Processa", oPanel,{|| Alert("Nใo disponํvel")/* ProcImpEx(@oMeter1,@oMeter2,@cHist,cDir,@lNaoFoi,@oGet2) */ },45,15,,,,.T.,,,,{|| lNaoFoi },,)
